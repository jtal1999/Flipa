...
async function searchProductMetrics(productDescription) {
    try {
        // Handle if productDescription is an object from Vision API
        ...
        // Extract the main product title/description
        const mainDescription = extractMainProductTitle(searchText);
        console.log('\n🔍 Main Product Description:', mainDescription);

        if (!mainDescription) {
            return {
                success: false,
                error: 'Could not extract product title from image'
            };
        }
